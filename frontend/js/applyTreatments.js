...
export async function openApplyTreatmentModal(diagnosisId, reloadCallback) {
  try {
    ...
    const treatmentListHtml = availableTreatments.map(t => `
...
searchInput.addEventListener('input', () => {
  ...
  options.forEach(option => {
    const name = option.querySelector('.treatment-option-name').textContent.toLowerCase();
    const keywords = option.querySelector('.treatment-option-keywords').textContent.toLowerCase();
...
resultsDiv.addEventListener('click', (e) => {
  ...
  if (option) {
    resultsDiv.querySelectorAll('.treatment-option').forEach(o => o.classList.remove('selected'));
    option.classList.add('selected');
    
    const treatmentId = option.dataset.id;
    const treatmentName = option.querySelector('.treatment-option-name').textContent;
...
const hideResultsHandler = (e) => {
  ...
  if (searchInput.contains(e.target) || resultsDiv.contains(e.target)) return; // Ignore clicks inside search area
  resultsDiv.style.display = 'none';
};
