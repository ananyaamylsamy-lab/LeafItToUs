...
async function init() {
  ...
  await Promise.all([diagnoses.loadDiagnoses(), treatments.loadTreatments()]);
...
export function updateStats() {
  document.getElementById("diagnosisCount").textContent =
    diagnoses.getDiagnosisCount();
  document.getElementById("treatmentCount").textContent =
    treatments.getTreatmentCount();
  document.getElementById("successRate").textContent =
    `${treatments.getAverageSuccessRate()}%`;
}
